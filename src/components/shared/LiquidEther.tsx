  ...
  useEffect(() => {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props) {
        ...
        const cx = props.cursor_size * props.cellScale.x, cy = props.cursor_size * props.cellScale.y
        u.center.value.set(Math.min(Math.max(Mouse.coords.x, -1 + cx + props.cellScale.x * 2), 1 - cx - props.cellScale.x * 2), Math.min(Math.max(Mouse.coords.y, -1 + cy + props.cellScale.y * 2), 1 - cy - props.cellScale.y * 2))
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscous, iterations, dt }) {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          this.uniforms.dt.value = dt; super.update()
        }
        return fbo_out
      }
    }
